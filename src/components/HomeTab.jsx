...
export default function HomeTab({ tasks, user, username, avatar, profilePic, theme, S, onComplete, onAddTask }) {
    ...
    <div style={{ padding: '20px 16px 110px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 12 }}>
          {/* Avatar */}
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: `linear-gradient(135deg,${theme.p},${theme.s})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, overflow: 'hidden', boxShadow: `0 0 10px ${theme.glow}` }}>
            {profilePic
              ? <img src={profilePic} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="avatar" />
              : avatar
            }
          </div>
          {/* Streak */}
          <div style={{ ...S.glass, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontSize: 17, fontWeight: 800, color: '#f1f5f9', fontFamily: "'Space Mono', monospace" }}>{user.streak}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...S.glass, padding: 16, marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <span style={{ ...S.neon, fontWeight: 700, fontFamily: "'Space Mono', monospace", fontSize: 14 }}>{doneCount}/{totalCount}</span>
        </div>
        ...
        {pct === 100 && <div style={{ marginTop: 10, textAlign: 'center', color: '#f59e0b', fontSize: 12, fontWeight: 700, letterSpacing: '1px' }}>✦ PERFECT DAY ACHIEVED ✦</div>}
      </div>
      ...
      {/* Weekly Challenges */}
      <div style={{ color: '#f1f5f9', fontWeight: 700, fontSize: 16, fontFamily: "'Syne', sans-serif", marginBottom: 12 }}>Weekly Challenges</div>
        ...
        {CHALLENGES.map((ch) => (
          <div key={ch.id} style={{ ...S.glass, padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ ...S.neon, fontSize: 12, fontFamily: "'Space Mono', monospace", fontWeight: 700 }}>{ch.prog}/{ch.goal}</span>
            </div>
      ...
      {/* Today's Tasks */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ color: '#f1f5f9', fontWeight: 700, fontSize: 16, fontFamily: "'Syne', sans-serif" }}>Today's Tasks</span>
